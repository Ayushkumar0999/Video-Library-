...
export function DeleteVideo() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (video) => {
      axios
        .put(`http://localhost:5050/edit-video/${params.id}`, video)
        .then(() => {
  ...
  function LoadCategories() {
    axios.get(`http://localhost:5050/get-categories`).then((response) => {
  ...
  function LoadVideos() {
    axios
      .get(`http://localhost:5050/get-video/${params.id}`)
      .then((response) => {
  ...
  function handleDeleteClick() {
    axios.delete(`http://localhost:5050/delete-video/${params.id}`).then(() => {
      console.log("deleted");
    });
    alert("Video Deleted");
    navigate("/admin-dashboard");
  ...
  return (
    <div>
      ...
      <Link to="/admin-dashboard" className="btn btn-warning mx-2">
        No
      </Link>
    </div>
  );
